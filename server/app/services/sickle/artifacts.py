...
class ArtifactWriter:
    ...
    def write_analysis(self, request_id: UUID, crop: dict, stage: dict, probabilities: np.ndarray, inputs) -> list[dict]:
        artifacts = [
            self.write_json(request_id, "crop_summary_json", crop),
            self.write_csv(request_id, "crop_summary_csv", [crop]),
            self.write_crop_geotiff(request_id, probabilities, inputs.field_mask, inputs.grid),
            self.write_crop_png(request_id, probabilities[0], inputs.field_mask),
            self.write_json(request_id, "stage_summary_json", {key: value for key, value in stage.items() if key != "timeline"}),
            self.write_csv(request_id, "stage_timeline_csv", stage.get("timeline", [])),
        ]
