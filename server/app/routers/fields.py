...
def retrieve_analysis(request_id: UUID, request: Request):
    ...
    crop = None if row["class_code"] is None else {
        key: row[key] for key in ("class_code", "class_label", "confidence", "paddy_probability", "non_paddy_probability", "paddy_pixel_fraction", "non_paddy_pixel_fraction", "field_pixel_count", "s1_observation_count", "s2_observation_count", "used_months", "rejected_months", "experimental")
    }
    stage = None if row["stage_status"] is None else {
        "status": row["stage_status"], "reason_code": row["reason_code"], "provisional": row["provisional"], "stage": row["stage"], "evidence": row["evidence"],
        "cycle_start": row["cycle_start"], "latest_observation": row["latest_observation"], "current_cycle_count": row["current_cycle_count"], "current_cycle_span_days": row["current_cycle_span_days"],
        "latest_ndvi": row["latest_ndvi"], "latest_ndmi": row["latest_ndmi"], "peak_confirmed": row["peak_confirmed"], "maximum_interpretation": row["maximum_interpretation"],
        "selected_s1_orbit_pass": row["selected_s1_orbit_pass"], "selected_s1_orbit_number": row["selected_s1_orbit_number"], "clean_s1_observation_count": row["clean_s1_observation_count"], "clean_s2_observation_count": row["clean_s2_observation_count"], "warning": row["stage_warning"],
    }
    ...
    result = {
        "request_id": row["request_id"], "field_id": row["field_code"], "geometry_revision": row["revision_number"], "status": row["status"],
        ...
        "charts": {"stage_timeline": repository.get_stage_chart(request_id)}, "warnings": row["warnings"], "artifacts": artifacts,
        "provenance": {"model": row["model_name"], "checkpoint_sha256": row["checkpoint_sha256"], "preprocessing_version": row["preprocessing_version"], "stage_rule_version": row["stage_rule_version"], "roi_version": row["roi_version"]},
    }
